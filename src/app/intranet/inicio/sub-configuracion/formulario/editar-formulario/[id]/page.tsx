  ...
  return (
    <div className="mb-4 p-4 border rounded">
      <Input
        ...
        onChange={(e) =>
          updateQuestion(String(question.idp), { nmPrg: e.target.value })
        }
        ...
        <div className="mb-2 text-slate-900 dark:text-slate-100">
          {question.opcs.map((option, index) => (
            <div key={index} className="flex mb-2">
              ...
              <Button
                onClick={() => {
                  const newOptions = question.opcs.filter(
                    (_, i) => i !== index
                  );
                  updateQuestion(String(question.idp), { opcs: newOptions });
                }}
...
export const DynamicForm: React.FC = () => {
  ...
  const [docsActivos, setDocsActivos] = useState<
    { idplantilladoc: number; nombre: string }[]
  >([]);
  ...
  useEffect(() => {
    const fetchQuestions = async () => {
      try {
        ...
        setNombre(data.name);
        setLongActivity(data.config.longActivity ?? false);
        setAllowDocLink(data.config.allowDocLink ?? false);
        setSelectedDoc(data.config.iddoc ?? null);
    ...
    const fetchDocs = async () => {
      try {
        const data = await DocumentosService.documentsActives();
        setDocsActivos(data);
      } catch (error) {
        console.error("Error al cargar documentos activos:", error);
      }
    };

    if (allowDocLink) fetchDocs();
  ...
  const saveForm = async () => {
    ...
    try {
      ...
      const formattedQuestions = formData.questions.map((question) => {
        const isNewQuestion =
          typeof question.idp === "string" && question.idp.startsWith("temp-");
        ...
        return {
          ...
          opcs: question.opcs.map((opt) => ({
            idOpc:
              typeof opt.idOpc === "string" && opt.idOpc.startsWith("temp-")
                ? undefined
                : opt.idOpc,
            txtOpc: opt.txtOpc,
          })),
      ...
      await FormularioService.updateForm(id, {
        ...
        iddoc: allowDocLink && selectedDoc ? selectedDoc : 0,
        longActivity,
        allowDocLink,
      });
      ...
      router.push(
        "/intranet/inicio/sub-configuracion/formulario?actualizado=true"
      );
    } catch (error) {
        ...
        <div className="mt-6 space-y-4 border rounded-md p-4">
          <div className="flex items-center justify-between">
            ...
            <input
              ...
              onChange={(e) => setLongActivity(e.target.checked)}
          ...
          <div className="flex items-center justify-between">
            ...
            <input
              ...
              onChange={(e) => setAllowDocLink(e.target.checked)}
              className="w-5 h-5 cursor-pointer"
            />
          </div>
          ...
          {allowDocLink && (
            <div className="mt-3">
              <Label>Selecciona un documento activo:</Label>
              <Select
                onValueChange={(value) => setSelectedDoc(Number(value))}
                value={selectedDoc ? String(selectedDoc) : undefined}
              >
